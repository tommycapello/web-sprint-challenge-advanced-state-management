...
- [done] Create a new working branch: git checkout -b `<firstName-lastName>`.
...
- [done] Adds the following state values into the initialState:

  - an array of smurfs
  - a boolean indicating if the app is loading
  - a string indicating a possible error message

- [done] Add in the arguments needed to complete a standard reducer function.
- [done] Add in a reducer case to accomidate the start of a smurf fetch.
- [done] Add in a reducer case to accomidate the successful smurf api fetch.
- [done] Add in a reducer cases to accomidate the failed smurf api fetch.
- [done] Add in a reducer case to accomidate adding a smurf (including the name, nickname, position, summary and an internally generated id) into your smurf list.
- [done] Add in a reducer case that adds in a value to the error message.
...
- [done] Add a thunk action called fetchSmurfs that triggers a loading status display in our application, performs an axios call to retreive smurfs from our server, saves the result of that call to our state and shows an error if one is made.
- [done] Add a standard action that allows us to add new smurf (including the name, nickname, position, summary).
- [done] Add a standard action that allows us to set the value of the error message slice of state.

#### App.js

Connect this component to redux **passing in your fetchSmurfs action only.**

- [done] Connect the fetchSmurfs actions to the App component.
- [done] Call the fetchSmurfs action when the component first loads.

#### components/SmurfList.js

Connect this component to redux, **passing in your smurf array and loading status state values.**

- [done] Connect the smurfs and loading state values to the SmurfList component.
- [done] Replace the single Smurf component instance with a map return a Smurf component for each entry in the smurfs list.
- [done] Replace the static isLoading variable with the state loading variable.
...
- [done] Connect the errorMessage, setError and addSmurf actions to the AddForm component.
- [done] Replace all instances of the errorMessage static variable with your error message state value.
- [done] Within the handleSubmit function, replace the static assignment to errorMessage with a call to the setError action. Test that an error is displayed when this validation code fails.
- [done] Within the handleSubmit function, call your addSmurf action with the smurf name, position, nickname and summury passed as arguments. Test that a smurf is correctly added to when the form is submitted.
